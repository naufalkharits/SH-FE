              ...
              <div key={tx.id} className="mb-5 space-y-6">
                <div className="flex gap-4 rounded-xl">
                  ...
                  <div className="w-full space-y-1">
                    ...
                    <div className="dark:text-white">{priceFormatter(tx?.product.price)}</div>
                    <div>
                      <span className="dark:text-white">Ditawar </span>
                      <span className="text-primary-purple-04">
                        {priceFormatter(tx?.price)}
                      </span>{" "}
                      <span className="dark:text-white">oleh</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <img className="h-8 w-8 rounded-full" src={tx?.buyer?.picture} alt="" />
                      <div className="">
                        <div className="text-xs  dark:text-zinc-50">{tx?.buyer?.name}</div>
                        <div className="text-xs text-neutral-04 dark:text-zinc-100">
                          {tx?.buyer?.city?.replace(/[^a-zA-Z\s]/g, "")}
                        </div>
                      </div>
                    </div>
                  </div>
